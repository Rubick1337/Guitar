...
def register_user(payload: schemas.UserCreate, db: Session = Depends(get_db)):
    print("📨 /auth/register called", payload.email)
    try:
        ...
        import traceback
        print("❌ ERROR in /auth/register:", repr(e))
        traceback.print_exc()
        # пробрасываем дальше, FastAPI вернёт 500
        raise


@router.post("/login", response_model=schemas.TokenResponse)
def login_user(payload: schemas.LoginRequest, db: Session = Depends(get_db)):
    print(f"🔐 /auth/login called for {payload.email}")
    try:
        email = payload.email.lower()
        ...
        import traceback
        print("❌ ERROR in /auth/login:", repr(e))
        traceback.print_exc()
        raise
